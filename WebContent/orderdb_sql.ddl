DROP TABLE IF EXISTS review;
DROP TABLE IF EXISTS shipment;
DROP TABLE IF EXISTS productinventory;
DROP TABLE IF EXISTS warehouse;
DROP TABLE IF EXISTS orderproduct;
DROP TABLE IF EXISTS incart;
DROP TABLE IF EXISTS product;
DROP TABLE IF EXISTS category;
DROP TABLE IF EXISTS ordersummary;
DROP TABLE IF EXISTS paymentmethod;
DROP TABLE IF EXISTS customer;
DROP TABLE IF EXISTS admin;


CREATE TABLE customer (
    customerId          INT IDENTITY,
    firstName           VARCHAR(40),
    lastName            VARCHAR(40),
    email               VARCHAR(50),
    phonenum            VARCHAR(20),
    address             VARCHAR(50),
    city                VARCHAR(40),
    state               VARCHAR(20),
    postalCode          VARCHAR(20),
    country             VARCHAR(40),
    userid              VARCHAR(20),
    password            VARCHAR(30),
    PRIMARY KEY (customerId)
);

CREATE TABLE admin(
    adminId     INT IDENTITY,
    firstName           VARCHAR(40),
    lastName            VARCHAR(40),
    userid              VARCHAR(20),
    password            VARCHAR(30),
    PRIMARY KEY (adminId)
);

insert into admin (firstName,lastName,userid,password) values ('main','admin','user','password');

CREATE TABLE paymentmethod (
    paymentMethodId     INT IDENTITY,
    paymentType         VARCHAR(20),
    paymentNumber       VARCHAR(30),
    paymentExpiryDate   DATE,
    customerId          INT,
    PRIMARY KEY (paymentMethodId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE
);

CREATE TABLE ordersummary (
    orderId             INT IDENTITY,
    orderDate           DATETIME,
    totalAmount         DECIMAL(10,2),
    shiptoAddress       VARCHAR(50),
    shiptoCity          VARCHAR(40),
    shiptoState         VARCHAR(20),
    shiptoPostalCode    VARCHAR(20),
    shiptoCountry       VARCHAR(40),
    customerId          INT,
    PRIMARY KEY (orderId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE
);

CREATE TABLE category (
    categoryId          INT IDENTITY,
    categoryName        VARCHAR(50),
    PRIMARY KEY (categoryId)
);

CREATE TABLE product (
    productId           INT IDENTITY,
    productName         VARCHAR(40),
    productPrice        DECIMAL(10,2),
    productImageURL     VARCHAR(100),
    productImage        VARBINARY(MAX),
    productDesc         VARCHAR(1000),
    categoryId          INT,
    PRIMARY KEY (productId) ,
    FOREIGN KEY (categoryId) REFERENCES category(categoryId)
        ON UPDATE CASCADE ON DELETE CASCADE
);
-- A Trigger to handle foreign key Constraint
Before insert on product			-- row lvl trigger
referencing new row as nR old row as oldR
When
	(nR.categoryId not in (Select categoryId from category))-- boolean Conditon
-- Do this if condition is true -> update/delete/insert
Insert into category (categoryName) values (nR.);

CREATE TABLE orderproduct (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE
);

CREATE TABLE incart (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE
);

CREATE TABLE warehouse (
    warehouseId         INT IDENTITY,
    warehouseName       VARCHAR(30),
    PRIMARY KEY (warehouseId)
);

CREATE TABLE shipment (
    shipmentId          INT IDENTITY,
    shipmentDate        DATETIME,
    shipmentDesc        VARCHAR(100),
    warehouseId         INT,
    PRIMARY KEY (shipmentId),
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE productinventory (
    productId           INT,
    warehouseId         INT,
    quantity            INT,
    price               DECIMAL(10,2),
    PRIMARY KEY (productId, warehouseId),
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE review (
    reviewId            INT IDENTITY,
    reviewRating        INT,
    reviewDate          DATETIME,
    customerId          INT,
    productId           INT,
    reviewComment       VARCHAR(1000),
    PRIMARY KEY (reviewId),
    FOREIGN KEY (customerId) REFERENCES customer(customerId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE
);



INSERT INTO category(categoryName) VALUES ('All Mountain Skis'); --category id 1
INSERT INTO category(categoryName) VALUES ('Racing Skis');--category id 2
INSERT INTO category(categoryName) VALUES ('Freeride Skis');--category id 3
INSERT INTO category(categoryName) VALUES ('Carving Skis');--category id 4
INSERT INTO category(categoryName) VALUES ('Park Snowboards');--category id 5
INSERT INTO category(categoryName) VALUES ('All Mountain Snowboards');--category id 6
INSERT INTO category(categoryName) VALUES ('Freeride Snowboards');--category id 7
INSERT INTO category(categoryName) VALUES ('Powder Snowboards');--category id 8

INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Instinct-Seeker', 1, '1 pair',1299.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Instinct-Wink',1,'1 pair',1399.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Instinct-Murrin',1,'1 pair',1399.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Redster G9',2,'1 pair',1250.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Vantage',4,'1 pair',975.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Capita The Outsiders',5,'Mens Snowboard',699.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Lib Tech Jamie Lynn',6,'Mens Snowboard',769.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Never Summer Proto Synthesis',6,'Mens Snowboard',669.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Gnu Gremlin',7,'Mens Snowboard',599.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Capita Navigator',8,'Mens Snowboard',599.00);


INSERT INTO warehouse(warehouseName) VALUES ('Main warehouse');
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (1, 1, 5, 1299.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (2, 1, 10, 1399.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (3, 1, 3, 1399.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (4, 1, 2, 1250.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (5, 1, 6, 975.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (6, 1, 4, 699.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (7, 1, 11,769.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (8, 1, 8,669.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (9, 1, 6,599.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (10, 1,3,599.00);

INSERT INTO warehouse(warehouseName) VALUES ('Secondary warehouse');
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (1, 2, 2, 1299.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (2, 2, 5, 1399.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (3, 2, 2, 1399.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (4, 2, 1, 1250.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (5, 2, 3, 975.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (6, 2, 2, 699.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (7, 2, 5,769.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (8, 2, 4,669.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (9, 2, 3,599.00);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (10, 2,2,599.00);


INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Arnold', 'Anderson', 'a.anderson@gmail.com', '204-111-2222', '103 AnyWhere Street', 'Winnipeg', 'MB', 'R3X 45T', 'Canada', 'arnold' , 'test');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Bobby', 'Brown', 'bobby.brown@hotmail.ca', '572-342-8911', '222 Bush Avenue', 'Boston', 'MA', '22222', 'United States', 'bobby' , 'bobby');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Candace', 'Cole', 'cole@charity.org', '333-444-5555', '333 Central Crescent', 'Chicago', 'IL', '33333', 'United States', 'candace' , 'password');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Darren', 'Doe', 'oe@doe.com', '250-807-2222', '444 Dover Lane', 'Kelowna', 'BC', 'V1V 2X9', 'Canada', 'darren' , 'pw');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Elizabeth', 'Elliott', 'engel@uiowa.edu', '555-666-7777', '555 Everwood Street', 'Iowa City', 'IA', '52241', 'United States', 'beth' , 'test');

-- Order 1 can be shipped as have enough inventory
DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (1, '2019-10-15 10:25:55', 3848.00)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 1, 1, 1299.00)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 2, 975.00)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 10, 1, 599.00);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-16 18:00:00', 975.00)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 1, 975.00);

-- Order 3 cannot be shipped as do not have enough inventory for item 7
DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (3, '2019-10-15 3:30:22',2596.00 )
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 6, 2, 699.00)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 9, 2, 599.00);



-- New images for lab 10
UPDATE Product SET productImageURL = 'img/instinct-seeker.png' WHERE ProductId = 1;
UPDATE Product SET productImageURL = 'img/instinct-wink.png' WHERE ProductId = 2;
UPDATE Product SET productImageURL = 'img/instinct-murrin.jpg' WHERE ProductId = 3;
UPDATE Product SET productImageURL = 'img/redster.jpg' WHERE ProductId = 4;
UPDATE Product SET productImageURL = 'img/vantage.jpg' WHERE ProductId = 5;
UPDATE Product SET productImageURL = 'img/outsiders.jpg' WHERE ProductId = 6;
UPDATE Product SET productImageURL = 'img/jamie.jpg' WHERE ProductId = 7;
UPDATE Product SET productImageURL = 'img/proto.jpg' WHERE ProductId = 8;
UPDATE Product SET productImageURL = 'img/gremlin.jpg' WHERE ProductId = 9;
UPDATE Product SET productImageURL = 'img/navigatpr.jpg' WHERE ProductId = 10;

INSERT INTO review(reviewRating,reviewDate,customerId,productId,reviewComment) VALUES(5,'2021-01-01',1,1,'This ski is fantastic');
INSERT INTO review(reviewRating,reviewDate,customerId,productId,reviewComment) VALUES(3,'2019-01-01',3,1,'Unsure about this ski');
INSERT INTO review(reviewRating,reviewDate,customerId,productId,reviewComment) VALUES(4,'2019-01-01',5,4,'It is fast! ');
INSERT INTO review(reviewRating,reviewDate,customerId,productId,reviewComment) VALUES(1,'2018-01-01',2,8,'Almost died.');
INSERT INTO review(reviewRating,reviewDate,customerId,productId,reviewComment) VALUES(5,'2022-01-01',3,5,'Brilliant design!');
INSERT INTO review(reviewRating,reviewDate,customerId,productId,reviewComment) VALUES(4,'2019-01-01',4,7,'Just Prime! ');

-- Loads image data for product 1
UPDATE Product SET productImage =  WHERE ProductId = 1;
